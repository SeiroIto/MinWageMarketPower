...
```{r copy FAD to saveddata, eval = F}
file.copy(
         from = paste0(pathdata,      "FAD.qs")
  ,        to = paste0(pathsaveddata, "FAD.qs")
  , overwrite = T
  )
```
...
library(tinytable)
aggsummary <- 
  qread(paste0(pathdata, "AggregateSummary.qs"), 
  nthreads = 13)
...
#### FA : FA of jobs
#### FAe: FA of employment/individuals
library(ggplot2)
library(cowplot)
...
#### )
gsz <- ggplot(data = FAdata[ii0 & !ii1 & !is.na(Size), ], 
  aes(x = FA)) +
...
irp5ForPlot <- unique(irp5Ma[taxyear >= 2010 & taxyear <= 2020, 
...
g1 <- ggplot(data = irp5ForPlot[busmainplc_geo != "", ], 
  ...
  facet_wrap(taxyear ~ .)
ggsave(
  paste0(pathdata, "NumberOfAgEmployees.jpg")
...
```{r copy LShareHHI to saveddata, eval = F}
file.copy(
         from = paste0(pathdata,      "LShareHHI.qs")
  ,        to = paste0(pathsaveddata, "LShareHHI.qs")
  , overwrite = T
  )
```
...
grepout <- function(str, x)
  # returns element of match (not numbers)
  x[grep(str, x, perl = T)]
idyr <- irp5[, .(taxyear, Txrf, busprov_geo, busdistmuni_geo, 
...
irp5[, TxUID := paste(Txrf, uid, sep = "-")]
setkey(irp5, TxUID); setkey(idyrW, TxUID)
irp5 <- idyrW[irp5, on = "TxUID"]
