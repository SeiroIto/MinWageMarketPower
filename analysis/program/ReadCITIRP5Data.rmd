...
Check how many firms there are with taxrefno == "" in stmfp (firm only) data

stmfp has number of employees per firm `irp5_kerr_iweight_b`
```{r check number of firms with empty taxrefno in stmfp}
ipyr <- qread(paste0(pathdata, "stmfp_2008_2022_e5_v1.qs"), nthreads = 8)
(tb <- table(ipyr[taxyear == 2012, .(empty = taxrefno == "")]))
round((tb/sum(tb))*100, 3)
(tb2 <- table(ipyr[taxyear == 2012, .(empty = taxrefno == "", irp5_kerr_iweight_b)][, 
  .(employment=sum(irp5_kerr_iweight_b, na.rm=T)), by = empty]))
#### employment share of empty taxrefno
round((1999293/10244729)*100, 2)
```

Confirm: firm entry = single row (no branch level info)
```{r check employment by total branch size}
ipyr[taxyear == 2012, c("BranchNum", "TotalBranch") := 
    list(
      1:.N,
      .N
    ),
  by = .(taxrefno)]
table(ipyr[, .(employment=sum(irp5_kerr_iweight_b, na.rm=T)), by = BranchNum])
```
