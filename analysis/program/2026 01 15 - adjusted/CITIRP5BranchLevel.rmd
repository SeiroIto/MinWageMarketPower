...
irp5[, LocGranular := factor(LocGranular, levels = GeoLevel)]
...
irp5[, MainPlNum := as.integer(length(unique(Mai))), by = .(Txrf, taxyear)]
irp5[, LocMunNum := as.integer(length(unique(Loc))), by = .(Txrf, taxyear)]
irp5[, DisMunNum := as.integer(length(unique(Dis))), by = .(Txrf, taxyear)]
irp5[, ProvNum   := as.integer(length(unique(Prv))), by = .(Txrf, taxyear)]
irp5[ProvNum   == 1L & 
   DisMunNum == 1L & 
   LocMunNum == 1L & 
   MainPlNum == 1L, 
...
irp5[, CommonLocality := factor(CommonLocality, levels = GeoLevel)]



#### Panel of various levels
#### Main place panel: If granular down to main place, or if there may be NAs in location for some rows but uniquely common locality level is at main place
irp5[grepl("^M", LocGranular) | grepl("^M", CommonLocality) & taxyear == 2009, 
  c("Txrf", geovars, "UID", "LocGranular", "CommonLocality"), with = F]
nrow(LSm <- LS[grepl("^M", LocGranular) | grepl("^M", CommonLocality), ])
nrow(LSl <- LS[grepl("^L", LocGranular) | grepl("^L", CommonLocality), ])
nrow(LSd <- LS[grepl("^D", LocGranular) | grepl("^D", CommonLocality), ])
nrow(LSp <- LS[grepl("^P", LocGranular) | grepl("^P", CommonLocality), ])

#### Missingness in location information
#### 1. Maximum sample size data sets
####   a. Main Place data = firms with information down to Main Place 
####   b. Loc Muni data   = firms with information down to Loc Muni    + a
####   c. Dis Muni data   = firms with information up to Dist Muni     + a + b
####   d. Prov data       = firms with information up to Prov          + a + b + c
#### 2. Same sample data sets
...
`IRP5_2012_cleaned.dta` of T:/IRP5/Job level/v5/beta/ &rarr; `irp12.qs` &rarr; `ipyrsClean.qs` (after dropping anomalous/invalid UID entries such as *, *, etc.) &rarr; `BranchWeights2012.qs`
