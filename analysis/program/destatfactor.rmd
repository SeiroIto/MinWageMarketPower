...
#  prob specifies quantiles, "short", "even", "odd", "very short", "long"
{
  ...
  if (is.character(prob))
  {
    ...
    {
      prob <- c(.025,.1,.25,.5,.75,.9,.975)
      y <- array(dim=c(ncol(x),14))
      dimnames(y) <- list(colnames(x), c("min","2.5\\%",
        "10\\%","25\\%","median","75\\%","90\\%","97.5\\%",
        "max","mean","std","0s","NAs","n"))
...
destatfactor <- function(x, prob = "short", signif = 1, removezero = F, IgnoreIfAbove = 10)
#  returns an array of descriptive statistics for factors/characters at all levels
#  If gender = M, F is a factor, it gives 2 rows of gender: M, gender: F 
#    prob: specifies quantiles, "short", "even", "odd", "long","very short"
#    IgnoreIfAbove: = 10, if length(levels(factor variable)) > 10, just skip
{
  if (is.null(dim(x))) dim(x) <- c(length(x),1)
  if (is.null(colnames(x))) colnames(x) <- paste("v", 1:ncol(x), sepv = "")
  ...
  iif <- sapply(xd, is.factor)
  iifc <- iic|iif
  #### Drop characters/factors with too many elements (> IgnoreIfAbove)
  if (any(iifc)) {
    Numunq <- unlist(lapply(xd[, ..iifc], function(z) length(unique(z))))
    if (!is.null(Numunq)) ii100 <- Numunq > IgnoreIfAbove else ii100 <- F
    if (any(ii100)) {
      message(paste(names(xd[, ..iifc])[ii100], if (length(ii100)==1) "has" else "have",  
        "more than", IgnoreIfAbove, "unique entries, dropped.\n"))
      #### drop iifc[ii100] columns, turn the rest to factors
      xf <- xd[, -which(iifc[ii100]), with = F]
      iic2 <- sapply(xf, is.character)
      xf[, (names(xf)[iic2]) := 
        lapply(.SD, as.factor), .SDcols = names(xf)[iic2]
      ]
    } else {  # if number of all character/factor element <= IgnoreIfAbove
      xf <- xd
      xf[, (names(xf)[iifc]) := 
        lapply(.SD, as.factor), .SDcols = names(xf)[iifc]
      ]
    }
  } else xf <- xd
  #### list all levels
  iif2 <- sapply(xf, is.factor)
  AllLvList <- lapply(xf[, ..iif2], levels)
  AllLvls <- unlist(AllLvList)
  AllNames <- unlist(lapply(1:length(AllLvList), 
    function(i) paste0(names(AllLvList)[i], ": ", AllLvList[[i]])))
  ...
  for (ii in 1:ncol(xf[, ..iif2])) {
    z <- xf[ , colnames(xf[, ..iif2])[ii], with = F]
    zn <- na.omit(z)
    if (removezero) zn <- z[!(is.na(z) | (!is.na(z) & z == 0))]
    NAs <- sum(is.na(z))
