...
#  path <- "c:/data/MinWageMarketPower/docs/Workshop2024Mar26/"; rmarkdown::render(paste0(path, "code/EstimationCode/R/RcodeCD_2024Mar25.Rmd"))
#### git rm --cached docs/Workshop2024Mar26/code/Appendix_DGP/DGP/DGPGameMA1AR1_1000x300_CD_20230426.csv
---
...
setwd("c:/data/MinWageMarketPower/")
getwd()
here::i_am("docs/Workshop2024Mar26/code/EstimationCode/R/RcodeCD_2024Mar25.Rmd")
getwd()
...
# "3rd degree is enough" by literature
degree <- 3
...
##  load data set
#### data_sim <- read_csv("data_simulated_20230426.csv") #wrong file name
data_sim <- read_csv("data_simulatedCD_20230426.csv")
...
# m_t: t=1, 2, 3, 4 for 5 period model, material expenditure defated by price index
# mu_t: markup
# pm_t: price index of materials, don't deflate with this if oligpsonic
# esp_t: MA demand shock
# a_t: unobservable aggregate quantity index of HSA demand
# Phi_t: total expenditure of the market
# u_t: transformed demand shocks
# 
...
FirstStepHorowitz <- function(df, degree, dfIV, degreeIV, data, L, lag){
  #### u_t, u_t-1: Need lag
  #### Kernel of Firpo et al.
  # Load data
  if (lag == 0){
    # use most recent period to do First Step Horowitz
    m <- data$m_4
    r <- data$r_4
    mL <- data$m_2 # IV
  }else{
  ...
  # default choice of position of knots
  ...
  # Boundaries of knots only for tau 
  Boundary_tau <- attr(bs_tau, "Boundary.knots")
  
  # pick each row and do kronecker products
  B <- row.kronecker(bs_m, bs_tau)
  # Can incorporate z demand shifters
  # B <- row.kronecker(row.kronecker(bs_m, bs_tau), bs_z)
 ...
 ## Derivatives
 dbs_m <- dbs(m_vec, 
              df = df, knots = knots_m,  # knots_m from bs results
              degree = degree, intercept = T)
 dbs_tau <- dbs(tau_vec, 
                df = df, knots = knots_tau, 
                degree = degree, intercept = T)
 # monotonicity constraint coefficients
 ...
 ## Constraint dB*alpha>0 => dB*alpha>=b(small number such as 0.001): b-dB*alpha<=0
  ...
  result_1st <- nloptr(x0 = a_ols,
                   eval_f = objective,
                   eval_grad_f = gr,
                   eval_g_ineq =  confun, # ineq constraints
                   eval_jac_g_ineq =  confun_jac,  # derivative of ineq constraints
                   opts = list("algorithm" = "NLOPT_LD_SLSQP", # sequential quad programming
                               "ftol_abs" = 1.0e-6,
                               "ftol_rel" = 1.0e-6,
                               "xtol_rel" =1.0e-8,
                               "xtol_abs" =1.0e-8,
 ...
 # need to use same parameters of main B spline functions
 bs_m0 <- bs(m_vec0,
             df = df, knots = knots_m,
             degree = degree, intercept = T)
...
# Store values from first step; using the estimated uhat values
  ...
  # dX_m1 aims to take derivative of the first stage revenue function
  ...
  # material expenditure share
  m_sh <- as.matrix(exp(data$pm_4 + data$m_4 - data$r_4))
  # derivative of 1st stage revenue func wrt m
  ...
  # this is required by matlab routine; aims to mimic matlab code
